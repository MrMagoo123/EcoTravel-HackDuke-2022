...
func CreateWorker(id string) (shared.AggregatorWorker, error) {
	return &greyhoundWorker{
		id:            "Greyhound " + id,
		browserConfig: data.NewConfig(),
	}, nil
}

type greyhoundWorker struct {
	id            string
	client        *http.Client
	browserConfig *data.BrowserConfig
}

func (r *greyhoundWorker) statusC(status string) {
	funcs.C2(r.id, status)
}

func (r *greyhoundWorker) Start() (err error) {
...
func (r *greyhoundWorker) Stop() error {
	return nil
}

func (r *greyhoundWorker) Query(query *shared.AggregatorQuery) ([]*shared.AggregatorResponse, error) {
